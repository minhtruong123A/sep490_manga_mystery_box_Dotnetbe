...
public class ExchangeRepository(
    ...
    : GenericRepository<ExchangeInfo>(context.GetCollection<ExchangeInfo>("ExchangeInfo")), IExchangeRepository
{
    ...
    private readonly IMongoCollection<MangaBox> _mangaBox = context.GetCollection<MangaBox>("MangaBox");
    private readonly IMongoCollection<Product> _product = context.GetCollection<Product>("Product");
    private readonly IMongoCollection<ProductInMangaBox> _productInMangaBox = context.GetCollection<ProductInMangaBox>("ProductInMangaBox");
    ...
    private readonly IMongoCollection<UserProduct> _userProduct = context.GetCollection<UserProduct>("User_Product");
        ...
        using (var session = await mongoClient.StartSessionAsync())
        {
            ...
            try
            {
                ...
                if (info.Status != (int)ExchangeStatus.Pending)
                    throw new Exception(
                        "This exchange request cannot be accepted because it is not in a pending state.");

                var sessionInfo = await _exchangeSession.Find(session, x => x.Id == info.ItemGiveId)
                    .FirstOrDefaultAsync();
                ...
                if (!sell.IsSell) throw new Exception("The product to be received no longer exists.");
                ...
                var sellProduct =
                    await _sellProduct.Find(session, x => x.Id == info.ItemReciveId).FirstOrDefaultAsync();
                ...
                if (updateSellResult.ModifiedCount == 0)
                    throw new Exception(
                        "Unable to process the exchange. The requested product may be out of stock or has been exchanged by another user.");

                var exchangeProducts =
                    await _exchangeProduct.Find(session, x => x.ExchangeId == sessionInfo.Id).ToListAsync();
                foreach (var ep in exchangeProducts)
                {
                    var userProduct = await _userProduct.Find(x => x.Id.Equals(ep.ProductExchangeId))
                        .FirstOrDefaultAsync();
                    if (userProduct.Quantity - ep.QuantityProductExchange < 0)
                        throw new Exception(
                            $"Not enough quantity for the product being offered: {ep.ProductExchangeId}");
                    ...
                    var userProductUpdate =
                        Builders<UserProduct>.Update.Inc(x => x.Quantity, -ep.QuantityProductExchange);
                    var updateUserProductResult =
                        await _userProduct.UpdateOneAsync(session, userProductFilter, userProductUpdate);
                ...
                foreach (var ep in exchangeProducts)
                {
                    var userProductToGive = await _userProduct.Find(session, x => x.Id == ep.ProductExchangeId)
                        .FirstOrDefaultAsync();
                    if (userProductToGive == null)
                        throw new Exception(
                            $"Could not find the user product with ID {ep.ProductExchangeId} that was offered.");

                    await UpdateUserProduct(session, sell.SellerId, userProductToGive.ProductId,
                        ep.QuantityProductExchange);
                }

                await _exchangeSession.UpdateOneAsync(session, x => x.Id == sessionInfo.Id,
                ...
                await userAchievementRepository.CheckAchievement(info.BuyerId);
                await userAchievementRepository.CheckAchievement(sell.SellerId);
                return true;
            }
            catch (Exception ex)
            {
    ...
    {
        var productInBox = await _productInMangaBox.Find(session, p => p.ProductId.Equals(productId))
            .FirstOrDefaultAsync();
        ...
        var filter =
            Builders<UserProduct>.Filter.Where(x => x.CollectorId.Equals(userId) && x.ProductId.Equals(productId));
    ...
    private async Task RejectIfExpiredAsync(List<ExchangeInfo> infos)
    {
        var expiredInfos = infos
            .Where(x => x.Datetime.AddDays(_settings.PendingExpireDays) < DateTime.UtcNow &&
                        x.Status == (int)ExchangeStatus.Pending)
            .ToList();
