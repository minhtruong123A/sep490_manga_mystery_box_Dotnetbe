...
public class SellProductRepository(
    ...
    IOptions<ProductPriceSettings> priceSettings,
    IUserAchievementRepository userAchievementRepository,
    IExchangeRepository exchangeRepository)
    : GenericRepository<SellProduct>(context.GetCollection<SellProduct>("SellProduct")), ISellProductRepository
{
    private readonly IMongoCollection<Collection> _collections = context.GetCollection<Collection>("Collection");
    private readonly FeeSettings _feeSettings = feeOptions.Value;
    private readonly IMongoCollection<MangaBox> _mangaBoxCollection = context.GetCollection<MangaBox>("MangaBox");
    private readonly IMongoCollection<OrderHistory> _orderHistoryCollection = context.GetCollection<OrderHistory>("OrderHistory");
    private readonly IMongoCollection<DigitalPaymentSession> _paymentSessionCollection = context.GetCollection<DigitalPaymentSession>("DigitalPaymentSession");
    private readonly ProductPriceSettings _priceSettings = priceSettings.Value;
    ...
    private readonly IMongoCollection<ProductOrder> _productOrderCollection = context.GetCollection<ProductOrder>("ProductOrder");
    ...
    private readonly IMongoCollection<UserCollection> _userUserCollection = context.GetCollection<UserCollection>("UserCollection");
    ...
    public async Task<int> CreateSellProductAsync(SellProductCreateDto dto, string userId)
    {
        ...
        if (dto.Price < _priceSettings.MinPrice || dto.Price > _priceSettings.MaxPrice)
            throw new Exception(
                $"Price must be between {_priceSettings.MinPrice:N0} VND and {_priceSettings.MaxPrice:N0} VND.");
        ...
        var updatedUserProduct =
            await _userProductCollection.Find(x => x.Id == dto.UserProductId).FirstOrDefaultAsync();
    ...
    public async Task<bool> UpdateSellProductAsync(UpdateSellProductDto dto)
    {
        ...
        if (sellProduct.IsSell) return false;
    ...
    {
        await exchangeRepository.RejectExchangeAutoWhenCancelSellProductAsync(sellProductId);
        ...
        var userProduct = await _userProductCollection
            .Find(x => x.ProductId.Equals(sellProduct.ProductId) && x.CollectorId.Equals(sellProduct.SellerId))
            .FirstOrDefaultAsync();
    ...
    public async Task<bool> ChangestatusSellProductAsync(string id)
    {
        ...
        if (sellProduct.Quantity <= 0) return false;
        if (sellProduct.IsSell)
            status = false;
        else
            status = true;
    ...
    public async Task<List<SellProductGetAllDto>> GetAllProductOnSaleAsync()
    {
        ...
        var productTask = _productCollection.AsQueryable().Where(c => productIds.Contains(c.Id.ToString()))
            .ToListAsync();
        var userTask = _userCollection.AsQueryable().Where(c => sellerIds.Contains(c.Id.ToString())).ToListAsync();
        var userProductTask = _userProductCollection.AsQueryable().Where(c => productIds.Contains(c.ProductId))
            .ToListAsync();
        var userCollectionTask = _userCollectionCollection.AsQueryable().Where(uc => sellerIds.Contains(uc.UserId))
            .ToListAsync();
        ...
        var collectionsTask = _collections.AsQueryable().Where(c => collectionIds.Contains(c.Id.ToString()))
            .ToListAsync();
        var raritiesTask = _rarityCollection.AsQueryable().Where(r => rarityIds.Contains(r.Id.ToString()))
            .ToListAsync();
    ...
    {
        //c.IsSell && 
        var sellProductList = await _sellProductCollection.AsQueryable()
            .Where(c => c.SellerId.Equals(id) && c.Quantity > 0).ToListAsync();
        var productIds = sellProductList.Select(c => c.ProductId).ToHashSet();
        var sellerIds = sellProductList.Select(c => c.SellerId).ToHashSet();
        var productTask = _productCollection.AsQueryable().Where(c => productIds.Contains(c.Id.ToString()))
            .ToListAsync();
        ...
        var collections = await _collections.AsQueryable().Where(c => collectionIds.Contains(c.Id.ToString()))
            .ToListAsync();
        var rarityIds = productList.Select(up => up.RarityId).ToHashSet();
        var rarities = await _rarityCollection.AsQueryable().Where(r => rarityIds.Contains(r.Id.ToString()))
            .ToListAsync();
    ...
    public async Task<SellProductDetailDto?> GetProductDetailByIdAsync(string id)
    {
        ...
        var productTask = _productCollection.AsQueryable()
            .FirstOrDefaultAsync(c => c.Id.ToString() == sellProduct.ProductId);
        ...
        var collectionTask = productResult?.CollectionId is string colId
            ? _collections.AsQueryable().FirstOrDefaultAsync(c => c.Id.ToString() == colId)
            : Task.FromResult<Collection?>(null);
        var rarityTask = productResult?.RarityId is string rId
            ? _rarityCollection.AsQueryable().FirstOrDefaultAsync(c => c.Id.ToString() == rId)
            : Task.FromResult<Rarity?>(null);
    ...
    public async Task<string> BuySellProductAsync(string buyerId, string sellProductId, int quantity)
    {
        ...
        var (buyerOrder, buyerHistory, buyerPayment) =
            await CreateBuyerRecordsAsync(buyerId, sellProduct, totalPrice, buyerWallet.Id);
        var (sellerOrder, sellerHistory, sellerPayment) =
            await CreateSellerRecordsAsync(seller.Id, buyerId, sellProduct, feeInfo, sellerWallet.Id);
        ...
        await userAchievementRepository.CheckAchievement(buyerId);
        return buyerHistory.Id;
    }

    public async Task<List<SellProductGetAllDto>> GetAllSellProductSuggestionsAsync(string userId)
    {
        var sellProductList = await _sellProductCollection
            .AsQueryable()
            .Where(sp => sp.IsSell && sp.Quantity > 0)
            .ToListAsync();
        if (!sellProductList.Any()) return new List<SellProductGetAllDto>();

        var userCollections = await _userUserCollection.Find(c => c.UserId == userId).ToListAsync();
        ...
        var userProducts = await _userProductCollection
            ...
            .ToListAsync();
        var ownedProductIds = userProducts.Select(up => up.ProductId.Trim()).ToHashSet();
        var filteredSellProducts = sellProductList
            .Where(sp => sp.SellerId != userId && !ownedProductIds.Contains(sp.ProductId))
            .ToList();
        if (!filteredSellProducts.Any()) return new List<SellProductGetAllDto>();

        var productIds = filteredSellProducts.Select(sp => sp.ProductId).ToHashSet();
        var sellerIds = filteredSellProducts.Select(sp => sp.SellerId).ToHashSet();
        var productTask = _productCollection.AsQueryable().Where(p => productIds.Contains(p.Id.ToString()))
            .ToListAsync();
        var userTask = _userCollection.AsQueryable().Where(u => sellerIds.Contains(u.Id.ToString())).ToListAsync();
        var userCollectionTask = _userCollectionCollection.AsQueryable().Where(uc => sellerIds.Contains(uc.UserId))
            .ToListAsync();
        await Task.WhenAll(productTask, userTask, userCollectionTask);
        ...
        var collections = collectionsTask.Result;
        var rarities = raritiesTask.Result;
        var result = filteredSellProducts.Select(sp =>
        {
            var product = productList.FirstOrDefault(p => p.Id.ToString() == sp.ProductId);
            var seller = userList.FirstOrDefault(u => u.Id.ToString() == sp.SellerId);
            var userCollection = userCollectionList.FirstOrDefault(uc => uc.UserId == sp.SellerId);
    ...
    {
        if (!product.IsSell || product.Quantity < quantity)
            throw new Exception("Not enough product quantity available for sale");
    }
    ...
    private async Task<SellProduct> GetSellProductAsync(string id)
    {
        return await _sellProductCollection.Find(x => x.Id == id).FirstOrDefaultAsync() ??
               throw new Exception("SellProduct not found");
    }
    ...
    private async Task<UseDigitalWallet> GetWalletAsync(string walletId)
    {
        return await _walletCollection.Find(x => x.Id == walletId).FirstOrDefaultAsync() ??
               throw new Exception("Wallet not found");
    }

    private (double rate, int fee, int net) CalculateFee(int total)
    {
        var rate = _feeSettings.BuyFeeRate;
        var fee = (int)(total * rate);
        return (rate, fee, total - fee);
    }

    private async Task ProcessWalletsAsync(UseDigitalWallet buyerWallet, UseDigitalWallet sellerWallet,
        (double rate, int fee, int net) feeInfo)
    {
    ...
    private async Task UpdateSellProductStockAsync(SellProduct product, int quantity)
    {
        ...
        if (result.ModifiedCount == 0)
            throw new Exception("The product is either out of stock or no longer available for purchase.");
    ...
    private async Task<(ProductOrder, OrderHistory, DigitalPaymentSession)> CreateBuyerRecordsAsync(string buyerId,
        SellProduct product, int total, string buyerWalletId)
    {
    ...
    private async Task<(ProductOrder, OrderHistory, DigitalPaymentSession)> CreateSellerRecordsAsync(string sellerId,
        string buyerId, SellProduct product, (double rate, int fee, int net) feeInfo, string sellerWalletId)
    {
        var order = new ProductOrder
        {
            SellerId = sellerId,
    ...
    private async Task LogTransactionFeeAsync(string fromUserId, string productId, string referenceId,
        (double rate, int fee, int net) feeInfo)
    {
    ...
    {
        var productInBox =
            await _productInMangaBoxCollection.Find(p => p.ProductId == productId).FirstOrDefaultAsync() ??
            throw new Exception("ProductInMangaBox not found");
        var mangaBox = await _mangaBoxCollection.Find(m => m.Id == productInBox.MangaBoxId).FirstOrDefaultAsync() ??
        ...
        var userCollection = await _userCollectionCollection
            .Find(c => c.UserId == buyerId && c.CollectionId == collectionId).FirstOrDefaultAsync();
        ...
        var userProduct = await _userProductCollection
            .Find(up => up.CollectorId == buyerId && up.ProductId == productId).FirstOrDefaultAsync();
        if (userProduct != null)
        {
