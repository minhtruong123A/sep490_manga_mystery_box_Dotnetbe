...
public class TransactionHistoryRepository(MongoDbContext context)
    : GenericRepository<TransactionHistory>(context.GetCollection<TransactionHistory>("TransactionHistory")),
        ITransactionHistoryRepository
{
    ...
    private readonly IMongoCollection<UserBank> _userBankCollection = context.GetCollection<UserBank>("UserBank");
    ...
    public async Task<List<TransactionHistoryDto>> GetTransactionsByWalletIdAsync(string walletId)
    {
        ...
        var filter = Builders<TransactionHistory>.Filter.And(
            Builders<TransactionHistory>.Filter.Eq(t => t.WalletId, walletId),
            Builders<TransactionHistory>.Filter.In(t => t.Type,
                new[] { (int)TransactionType.Recharge, (int)TransactionType.Withdraw })
        );
