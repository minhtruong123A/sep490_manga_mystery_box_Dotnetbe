                ...
                var payload = new
                {
                    contents = new[]
                    {
                        new
                        {
                            parts = new[]
                            {
                                new { text = $"Is the following message toxic, offensive, or inappropriate? Answer 'True' if yes, 'False' if safe. Message: \"{content}\"" }
                            }
                        }
                    }
                };
                if (string.IsNullOrWhiteSpace(_apiGeminiKey)) throw new InvalidOperationException("Gemini API key is not configured.");

                var request = new HttpRequestMessage(HttpMethod.Post,$"https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent?key={_apiGeminiKey}")
