    ...
    : IReportService
{
    ...
    {
        return await uniUnitOfWork.ReportRepository.CreateReportAsync(dto, userId);
    }

    public async Task<bool> UpdateStatus(string reportId)
    {
        var report = await uniUnitOfWork.ReportRepository.GetByIdAsync(reportId);
        if (report == null) throw new Exception("Report not exist");
        report.Status = true;
        await uniUnitOfWork.ReportRepository.UpdateAsync(reportId, report);
        return true;
    }

    public async Task<List<ReportResponeDto>> GetAllReportAsync()
    {
        return await uniUnitOfWork.ReportRepository.GetAllReportAsync();
    }

    public async Task<List<ReportResponeDto>> GetAllReportOfUserAsync(string userId)
    {
        return await uniUnitOfWork.ReportRepository.GetAllReportOfUserAsync(userId);
    }

    public async Task<SalesReportDto> GetSalesReportAsync(string userId)
    {
        var allOrderHistory = await orderHistoryService.GetOrderHistoryAsync(userId);
        ...
        {
            var productInfo = await productService.GetProductWithRarityByIdAsync(item.ProductId);
